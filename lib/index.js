...
export default function contributin ({
  ...
  app.get("/", (req, res) => {
    ...
    let page = dom("html",
      ...
      splash({ coc, path, css, name, owner, logo, active, total })
    );
    res.type("html");
  ...
  //   }
  //
  //   invite({ token, owner, email, channel: chanId }, err => {
  //     if (err) {
  //       if (err.message === `Sending you to GitHub...`) {
  //         return res
  //         .status(303)
  //         .json({ msg: err.message, redirectUrl: `https://${owner}.slack.com` });
  //       }
  //
  //       return res
  //       .status(400)
