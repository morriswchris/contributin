...
function style ({ logo, large, iframe } = {}){
  var css = dom.style();
...
export default function splash ({ path, repo_name, coc, logo, stars, issues, large, iframe }){
  let div = dom(".splash",
    !iframe && dom(".logos",
      logo && dom(".logo.org"),
      dom(".logo.github")
    ...
    dom("p",
      "Become a contributor of ", dom("b", repo_name),
      " on GitHub."
    ),
    dom("p.status", [
        dom("b.active", stars), " Stars ",
        dom("b.total", issues), " Issues"
      ]
    ),
    dom("form id=invite",
      dom("input.form-item type=input name=username placeholder=\"Your GitHub username\"" + (!iframe ? "autofocus" : "")),
      coc && dom(".coc",
        dom("label",
      ...
      dom("button.loading", "Start Contributin")
    ),
    !iframe && dom("footer",
      "powered by ",
      dom("a href=http://morriswchris.github.io/contributin target=_blank", "contributin")
    ),
    style({ logo, large, iframe }),
    // xxx: single build
    dom("script", `
      data = {};
